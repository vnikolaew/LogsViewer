...
const Home = ({}: HomeProps) => {
   ...
   function handleSubscribe() {
      const logFile = tree
         .find(r => r.serviceName === selectedServiceName.trim())
         ?.logFiles?.[0];

      if (logFile && selectedServiceName) {
         router.push(`/${selectedServiceName.trim()}/${logFile.fileName}`);
      }
   }
